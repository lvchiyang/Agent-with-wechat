...
class ChromaDBManager:
    def __init__(self, path: str = "Agnet_ChromDB", embedding_dim: int = 1024):
        """初始化数据库连接"""
    ...
    def time_search(self, collection: chromadb.Collection, timestamp: int = None) -> list[str]:
        results = collection.query(
            query_texts=[""],
            where={"id": {"$gt": timestamp}}, # 这一可以过滤掉群聊，只整理个人信息
        )
        return results['documents'][0]
    
    def id_search(self, collection: chromadb.Collection, id: int = 1) -> str:
        """个人信息"""
...
def test_chromadb_manager():
    ...
    try:
        ...
        # 测试个人信息搜索
        search_results = chroma_db.id_search(coll)
