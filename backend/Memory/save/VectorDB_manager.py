...
from datetime import datetime, timedelta
from backend.Memory.save.VectorDB import VectorDBConfig, VectorDBFactory
'''
...
class MemoryManager:
    def __init__(self, LLM_Client):
        ...
        # 初始化向量数据库
        db_config = VectorDBConfig(db_type="chromadb", db_path=str(lance_path))
        self.vector_db = VectorDBFactory.create_db(db_config)
        # 移除非线程安全的事件循环代码
    ...
    def _archive_context(self, context: List[Dict]):
        ...
        try:
            ...
            if self.last_interacted["group_name"] == "群聊":
                ...
                collection = self.vector_db.create_collection(table_name)
            ...
            self.vector_db.add_data(collection, {
                "id": int(time.time()),
                "vector": embedding,
                "text": context_str,
    ...
    def query_context(self, message: dict = None) -> List[Dict]:
        ...
        try:
            ...
            table_name = self.vector_db.name_to_pinyin(table_name)

            # 检查表是否存在
            # if table_name not in self.lance_db.list_tables():
            if table_name not in self.vector_db.list_collections():
                logger.info(f"记忆中尚且没有和{table_name}相关的信息")
                return [{"记忆中尚且没有相关信息"}]
            else:
                # table = self.lance_db.open_or_create_table(table_name)
                collection = self.vector_db.open_or_create_collection(table_name)
                ...
                # 统一接口调用
                results = self.vector_db.query_vector(
    ...
    async def daily_summary(self):
        ...
        tables = self.vector_db.list_collections()
        ...
        for table_name in tables:
            ...
            today_results = self.vector_db.time_search(collection, yesterday_end)

            # 查询个人信息
            # profile = self.lance_db.table_search(table, filter = f"id = 1")
            profile = self.vector_db.id_search(collection, 1)
